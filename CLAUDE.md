...
- `topic` / `topics` — flat, name-keyed bundles of meaning (core; derived views in `arctx.core.topics`, no topic record). `topic tag NAME ID...` marks any nodes/steps as belonging to a subject — connectivity is NOT required and is never validated; the view groups tagged records into islands (connected components over the active graph), and 2+ islands is a join *candidate*, not an error. `topic summarize NAME --summary TEXT [--source ID]` is the subject's current statement (generic payload `type="topic_summary"`; latest by record_event_rank wins, append-order tie-break). `guide --context` prints the top current statements. Tags/summaries are plain generic payloads (`type="tag"`) so old readers degrade gracefully.
