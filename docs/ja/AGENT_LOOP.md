...
子プロセスには `spawn` を使います。子は固有の `ARCTX_WORK_SESSION_ID` を受け取り、
...
明示モードでは、変更系コマンドごとに `--run` と `--work-session` の両方を渡します。

```bash
arctx add step --run run_x --work-session ws_xxx --from NODE_ID --type implementation
```

デフォルトの attribution は `user=user`, `work_session=default` です。誰がどの
