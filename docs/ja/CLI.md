...
| DAG 操作 | [``rewind``](#optagent-rewind) | observed transition を1本切り、current を切り口に戻す（履歴は消さない） |
|          | [``refresh``](#optagent-refresh) | PredictionDAG を current state に再アンカーする |
...
指定した observed transition を **cut** します。**TraceDAG の state / transition / plan / result はひとつも消えません**。書き換わるのは ``current_observed_state_id``（cut した transition の ``from_observed_state_id`` に移動）と、TraceDAG に append される **1本の ``TraceCut`` レコード** だけです。

cut 後の最初の ``observe`` / ``promote`` は、cut の起点（active な親）から新しい兄弟枝として枝分かれします。

```bash
optagent rewind <transition_id> [options]
```
...
| 引数 | 必須 | 説明 |
|-----|------|------|
| ``transition_id`` | ○ | cut する observed transition の ID（current から後ろ向きに辿って到達できる必要あり） |
...
- ``ValueError`` — ``transition_id`` が current の active path 上にない場合（兄弟枝など）、または既に cut されている場合
