...
Phase 1 の途中で `RunGraph.views` と `view` CLI は削除した。旧 `views` record は新しい core graph には取り込まない。

移行方針:

1. CLI / docs から `view` を外す。
2. 新しい主要 CLI は `RunGraph` 全体を対象にする。
3. storage schema から `views` を削除する。
...
`transition`, `payload`, `node` はトップレベルCLI登録から外した。`graph dump` は互換CLIとして当面残し、主要ドキュメントでは新CLIを前面に出す。
