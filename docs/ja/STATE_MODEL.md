...
status は lane record 上の可変フィールドではなく、append-only な `WorkEvent`
（`lane_closed` / `lane_opened`）の **projection** です。lane record はディスク上で
最初に書かれたまま不変で、現在の status は load 時に各 lane の最新イベントを畳んで
...
有効な cut 状態は読み取り時に「最後の cut/uncut marker が勝つ」で算出されます
（`arctx.core.cuts`）。同一対象を cut → uncut → cut と繰り返しても全系列が残ります。
...
- cut 軸は実装済み: `UncutPayload` ＋ `RunHandle.uncut(...)`。誤 cut は `uncut` の
  **追記**で回復する（元の cut は削除しない＝append-only を守ったまま誤操作を救う、
  復活は無損失）。step の uncut は「output node に active producer を2つ作らない」
  不変条件で守る（reparent で cut した旧 producer をうっかり復活させない）。
