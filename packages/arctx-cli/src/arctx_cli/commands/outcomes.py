...
def add_parser(subparsers) -> argparse.ArgumentParser:
    parser = subparsers.add_parser("outcomes", help="List output nodes for a Step")
    parser.add_argument("step_id", help="Step to inspect")
...
def run_outcomes_command(
    *,
    ...
    result = handle.outcomes(step_id)

    if include_payloads:
        result["step_payloads"] = [
            p.to_dict() for p in g.payloads_for_step(step_id)
        ]
        result["output_nodes"] = [
            {
