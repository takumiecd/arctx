...
def add_parser(subparsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--step", dest="step_id", default=None)
    parser.add_argument("--payload", dest="payload_id", default=None)
    parser.add_argument("--with-payloads", action="store_true")
    parser.add_argument(
        "--outputs", action="store_true", help="(with --step) include output nodes"
    )
    parser.add_argument(
        "--history",
        action="store_true",
        help="(with --step) show all GitChangePayload entries in append order",
    )
    ...
    *,
    ...
    if kind == "node":
        ...
        return {
            ...
            "incoming_step_id": graph.step_to_node(record_id),
            "outgoing_step_ids": graph.steps_from_node(record_id),
        }

    if kind == "step":
        step = graph.steps[record_id]
        return {
            "kind": "step",
            "id": record_id,
...
def run_show_command(
    *,
    ...
    if outputs and step_id is None:
        raise ValueError("--outputs can only be used with --step")
    ...
    if step_id is not None:
        step = g.steps.get(step_id)
        if step is None:
            raise KeyError(f"unknown step_id: {step_id}")
        result = {
            "step": step.to_dict(),
            "input_node_ids": g.step_inputs(step_id),
        }
        if with_payloads:
            result["payloads"] = [p.to_dict() for p in g.payloads_for_step(step_id)]
        if outputs:
            result["output_nodes"] = [
