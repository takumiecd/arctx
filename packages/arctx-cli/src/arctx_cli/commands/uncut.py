"""arctx CLI uncut command — append-only reversal of a cut."""
...
def add_parser(subparsers) -> argparse.ArgumentParser:
    parser = subparsers.add_parser("uncut", help="Reverse a cut on a Node or Step")
...
def cli_uncut(args) -> int:
    ...
    if args.node_id is not None:
        target_id, target_kind = args.node_id, "node"
    elif args.step_id is not None:
        target_id, target_kind = args.step_id, "step"
    elif args.kind is not None and args.id is not None:
        if args.kind not in ("node", "step"):
            raise ValueError("uncut target kind must be node or step")
        target_id, target_kind = args.id, args.kind
