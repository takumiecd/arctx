"""Tests for arctx hook post-merge functionality."""
...
from arctx_cli.ext.git.hook import run_hook_install, run_hook_post_merge
...
def _init_arctx(tmp_path: Path, run_id: str = "run_pm_test") -> dict:
...
class TestRunHookPostMerge:
    ...
    def test_skip_already_known_sha(self, tmp_path):
        """If HEAD sha is already in arctx, action is skip."""
        from arctx_cli.ext.git.commit import run_commit_command

        _init_arctx(tmp_path, run_id="run_pm_known")
        sd = _store_dir(tmp_path)

        # Record a commit in arctx with a known sha.
        run_commit_command(
            message="known",
            branch="main",
            run_id="run_pm_known",
    ...
    def test_adopt_merge_when_other_node_known(self, tmp_path):
        ...
        _init_arctx(tmp_path, run_id="run_pm_adopt")
        sd = _store_dir(tmp_path)

        # Build two branches in arctx.
        run_commit_command(
            message="main",
            branch="main",
            run_id="run_pm_adopt",
        ...
        handle.run_graph.add_lane(
            Lane(lane_id="ws_f", run_id=handle.run_id, created_by="u")
        )
