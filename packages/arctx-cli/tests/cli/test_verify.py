"""Integration tests for arctx verify CLI.

Two flavours:
1. Real git repo tests: arctx init → arctx commit → arctx verify (actual git calls).
...
from arctx_cli.ext.git.verify import run_verify_command
...
def _init_arctx(tmp_path: Path, run_id: str = "run_test") -> dict:
...
class TestVerifyCLIMocked:
    def _setup_run(self, tmp_path, run_id: str = "run_m"):
        """Create a arctx run with a two-commit chain in a temp store."""
