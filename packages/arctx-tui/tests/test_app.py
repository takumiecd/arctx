...
def test_app_has_editor_actions_bound():
    ...
    src = (pathlib.Path(__file__).parent.parent / "src/arctx_tui/app.py").read_text()
    tree = ast.parse(src)
    method_names = {
...
def test_tui_main_exposes_watch_flags():
    ...
    src = (pathlib.Path(__file__).parent.parent / "src/arctx_tui/main.py").read_text()
...
def test_outline_in_tui_scenario():
    from arctx.core.run.dump import DumpOptions, render_outline
    handle = _make_handle()
...
def test_mermaid_in_tui_scenario():
    from arctx.core.run.dump import DumpOptions, render_mermaid
    handle = _make_handle()
