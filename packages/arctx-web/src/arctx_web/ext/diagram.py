...
from arctx_web.extensions import WebExtensionBase


class DiagramWebExtension(WebExtensionBase):
    """Browser-side diagram payload preview."""

    def scripts(self) -> list[str]:
        """Return the browser renderer for diagram payloads."""
...
(function () {
  ...
  class DiagramPreview extends HTMLElement {
    ...
    render() {
      ...
      const nodeHtml = nodes
        .map((node) => `<span class="node">${escapeHtml(labelForNode(node))}</span>`)
        .join("");
      const edgeHtml = edges
        .map((edge) => `<span class="edge">${escapeHtml(labelForEdge(edge))}</span>`)
        .join("");
  ...
  function labelForNode(node) {
    ...
    return node.label || node.name || node.id || JSON.stringify(node);
  }
  ...
  function install(api) {
    if (!customElements.get(tagName)) customElements.define(tagName, DiagramPreview);
    api.registerPayloadElement("diagram", { tagName });
  }
