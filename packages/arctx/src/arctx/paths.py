...
The jsonl/json canon is the run — it is what gets committed. Derived local
files (``run.cache.pkl``, lock/temp files) are excluded by the
...
GITIGNORE_LINES = (
    ...
    # Pre-0.4.6 caches. The cache lives outside the run directory now (see
    # arctx.storage._cache); a leftover here is deleted on sight, never read.
    "run.cache.pkl",
    ".append.lock",
    ".append.journal",
    ".cache_tmp_*",
