...
class TestCherryPickPayload:
    ...
    def test_to_dict_round_trip(self):
        ...
        assert restored.source_step == p.source_step
        assert restored.source_commit == p.source_commit
        assert restored.metadata == p.metadata

    def test_to_dict_round_trip_none_source_step(self):
        p = CherryPickPayload(
            payload_id="pl_cross",
            target_id="t_cross",
            source_step=None,
        ...
        d = p.to_dict()
        assert d["source_step"] is None

        restored = payload_from_dict(d)
        assert isinstance(restored, CherryPickPayload)
        assert restored.source_step is None
