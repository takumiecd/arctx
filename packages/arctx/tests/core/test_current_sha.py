...
def _make_graph_with_step() -> tuple[RunGraph, str]:
    """Return a RunGraph and one step_id."""
    g = RunGraph()
...
class TestStepBySha:
    def test_not_found_returns_none(self):
        g, t_id = _make_graph_with_step()
        assert step_by_sha(g, "deadbeef") is None

    def test_found_returns_step_id(self):
        g, t_id = _make_graph_with_step()
        _git_payload(g, t_id, "pl_1", "cafecafe")
        assert step_by_sha(g, "cafecafe") == t_id
    ...
    def test_after_amend_old_sha_not_found(self):
        ...
        assert step_by_sha(g, "old_sha") is None
        assert step_by_sha(g, "new_sha") == t_id

    def test_multiple_steps_returns_latest(self):
        """When two steps somehow share a sha, return the last inserted."""
        g = RunGraph()
        n0 = Node(node_id="n0")
        n1 = Node(node_id="n1")
        n2 = Node(node_id="n2")
        ...
        # Both have current_sha == shared_sha; step_by_sha returns the last
        # one visited (most-recently-created, i.e. t_second).
        result = step_by_sha(g, sha)
