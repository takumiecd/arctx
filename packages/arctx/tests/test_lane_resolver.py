...
from arctx.session import resolve_lane_id
...
def test_lane_resolved_from_file_pointer(tmp_path, monkeypatch):
    ...
    assert resolve_lane_id(None) == "lane_xyz"
...
def test_lane_resolved_from_run_scoped_file_pointer(tmp_path, monkeypatch):
    ...
    assert resolve_lane_id(None, run_id="run_a") == "lane_run_a"
    assert resolve_lane_id(None, run_id="run_b") == "lane_run_b"
...
def test_run_scoped_pointer_beats_legacy_file_pointer(tmp_path, monkeypatch):
    ...
    assert resolve_lane_id(None, run_id="run_x") == "lane_run"
...
def test_env_beats_file_pointer(tmp_path, monkeypatch):
    ...
    assert resolve_lane_id(None, run_id="run_x") == "lane_env"


def test_legacy_lane_env_is_fallback(tmp_path, monkeypatch):
    _fake_repo(tmp_path, monkeypatch)
    monkeypatch.setenv("ARCTX_LANE_ID", "lane_legacy_env")
    assert resolve_lane_id(None) == "lane_legacy_env"
