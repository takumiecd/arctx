...
- optagent dump: run 全体を outline または mermaid でレンダリングする。
...
optagent observe --run demo it_0001 --status completed --metric latency_ms=1.5
optagent dump --run demo
```

既定では run は .optagent/runs 以下に保存されます。

サブトピック一覧を確認するには `optagent guide --list` を実行してください。
""",

    "agent": """\
# optagent agent guide — LLM 向け利用原則

LLM が optagent を使う場合の原則と anti-pattern をまとめます。
...
**ID は手で組み立てない**
optagent が返す ID (it_XXXX, ot_XXXX, n_XXXX 等) をそのまま使います。
...
**状態確認は dump 1 発で行う**
`optagent show` を何度も呼ぶよりも `optagent dump` で全体を取得する方が
...
```bash
# 1. run を初期化する
optagent init <requirement_id> --run-id <rid>

# 2. plan を作る
optagent plan --run <rid> --input-node n_0000 --intent "..."
...
# mermaid で図を生成
optagent dump --run demo --format mermaid

# observed のみ、深さ 3 まで
optagent dump --run demo --observed-only --depth 3
```
""",
...
# 3. optagent の外で実際に実行する
# (ここでベンチマーク、実験、評価などを実行)

# 4. 結果を observe として記録する
optagent observe --run demo it_0001 \\
  --status completed \\
...
# 5. 軽いメモを残す (任意)
optagent note --run demo --node n_0001 --text "baseline confirmed, latency within target"
...
```bash
optagent plan --run demo --input-node n_0001 --intent "apply loop unrolling"
# -> it_0002

optagent predict --run demo it_0002 --max-outcomes 1
# -> ot_0002 (PredictionPayload 付き) が作られる

# (ここで実際に実行)

optagent observe --run demo it_0002 \\
...
- CLI: optagent observe で作られる OutputTransition に自動 attach される。
...
attach されます。optagent observe は常に新しい OutputTransition を作成するため、
...
optagent records optimization and problem-solving processes as an append-only history graph.
...
- optagent dump: render the whole run as outline or mermaid.
...
Runs are stored under .optagent/runs by default.

Run `optagent guide --list` to see all available subtopics.
""",

    "agent": """\
# optagent agent guide — rules for LLM agents
...
Use IDs returned by optagent commands (it_XXXX, ot_XXXX, n_XXXX).
...
A single `optagent dump` returns the full run context in one token-efficient block.
Multiple `optagent show` calls waste tokens and give a fragmented picture.
...
optagent note --run demo --node n_0001 --text "baseline confirmed"
...
ResultPayload
  ...
  CLI: attached automatically by optagent observe.
...
optagent observe always creates a new OutputTransition with a ResultPayload.
It never overwrites the OutputTransition created by optagent predict.
...
TOPIC_SUMMARIES: dict[str, str] = {
    ...
    "agent":     "Rules and anti-patterns for LLM agents using optagent",
...
def cli_guide(args) -> int:
    ...
    try:
        ...
        print("Run `optagent guide --list` to see available topics.", file=sys.stderr)
        return 1
